...
Pagination links can be generated by overriding the `JSONAPI.View.pagination.links/4` callback of your view and returning a map containing the links.
...
Links can be generated using the `JSONAPI.Config.page` information store in the connection assign `jsonapi_query` and by passing additional information to the `pagination_links/4` callback or your paginator module by passing `options` from your controller.
