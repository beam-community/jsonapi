defmodule JSONAPI.Utils.List do
  ...
  ## Examples
      ...
      iex> to_list_of_query_string_components(%{"filters" => %{"age" => %{"eq" => 18}, "name" => %{"cont" => "John"}}})
      [{"filters[age][eq]", 18}, {"filters[name][cont]", "John"}]
  ...
  defp do_to_list_of_query_string_components({key, value}) when is_map(value) do
    Enum.flat_map(value, fn {k, v} -> do_to_list_of_query_string_components({"#{key}[#{k}]", v}) end)
  end
